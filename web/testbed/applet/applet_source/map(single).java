...
implements MouseListener, MouseMotionListener 
{
			...
			case 0:
				...
				/*else
				{
					for(int i = 0;i < vnodes.size();i++)
					{
						int tempx = ((nodes)vnodes.get(i)).getx();
						int tempy = ((nodes)vnodes.get(i)).gety();
						if(mx > (int)(width*0.16)+presssx && mx < (int)(width*0.16)+pressex+14 && my > (int)(height*0.03)+presssy && my < (int)(height*0.03)+pressey+14)
						{
							action = 1;
							submenux=mx; submenuy= my;
							nodeselected.add(i);
	...
	public void paint( Graphics g ) 
	{
		...
		for(int i = 0;i < vnodes.size();i++)
		{
			...
			if((action== 0||action==1)&&(nodeselected == i||nodeover == i))
