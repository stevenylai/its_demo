...
{
	//$test++;
	$sql ="select AVG(DTemp) as avg, DMoteID from listener, data where LID =".$_REQUEST['LID']." and LID = DLID  and DTime>'$time' and  DTime<='".addSecondsToTimestamp($time,$interval)."' group by DMoteID ";
	$result = $db->query($sql, true);
...
//} while($time < $max && $test <100);
} while($time < $max);

$strXML ="<chart caption='Temperature' subCaption='' numdivlines='9' lineThickness='2' showValues='0' anchorRadius='3' anchorBgAlpha='50' formatNumberScale='0' labelStep='4'  labelDisplay='WRAP' yAxisMinValue='".((int)($amin-$amin/100*20))."' yAxisMaxValue='".((int)($amax+$amax/100*20))."'  showAlternateVGridColor='1' numVisiblePlot='12' animation='0'>";
$strXML .="<categories >";
$strXML .=$category ;
$strXML .="</categories>";
$color[1] = 800080;
