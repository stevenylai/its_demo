...
public class MessageHandler implements Runnable, SensorConstants
{
	...
	private void processMessage () throws UnknownMessageException
	{
		...
		System.out.println("get Msg hahah" + (msg instanceof ITSMsg));
		...
		else if (msg instanceof OscopeMsg)
		{
		...
		else if(msg instanceof ITSMsg)
		{
			ITSMsg smsg = new ITSMsg(msg.dataGet(),0);//((MultiHop)msg).offset_data(0));
			...
			turningDirection=smsg.get_direction();
		...
		/*	// Mote is uncovered! s.getLastData() == SENSOR_COVERED &&
			if ( moteData == SENSOR_COVERED)
			{
				Road r = s.getRoad();
				double pos = s.getPos();
				try 
				{
					...
					if (c!=null)
					{
						// Set the new position for the car
						if (r.getType()==FIFO_ROAD && c.getPos() < 1)
							c.setPos (pos+0.1);
						else 
						{
							if (c.getDirection()==DIRECTION_FORWARD)
								c.setDirection(DIRECTION_BACKWARD);
							if(c.getPos() > 0)
								c.setPos (pos-0.1);
						}
						calculateSpeed (c, s, r);
						checkSafety(c, s, r);
						s.addData(moteData, c);
						// Check if the car has passed the end of a road
						if (r.getType()==FIFO_ROAD && r.getNextSensor(s) == null)
						{
							// Move the car into the cross road
							CrossRoad cross = r.getEndCrossPoint();
							if (cross != null)
								cross.addCar(c, r);
						}
						if (r.getType()==LIFO_ROAD && r.getPreviousSensor(s) == null)
						{
							// Move the car into the cross road
							CrossRoad cross = r.getStartCrossPoint();
							if (cross != null)
								cross.addCar(c, r);
						}
					}
					...
					// in case car is not found, just ignore because the data should be noise
				{
					System.out.println ("Couldn't find any car, message is ignored: " +moteID+", "+moteData);
				}
			} 
			else
				System.out.println("No change in data, ignored...");
			s.setLastData(moteData);
			dumper.dump();
		}
		/*
		//else if (msg instanceof MultiHop)
		{
			System.out.println("Trying to extract ITSMsg");
			//ITSMsg smsg = new ITSMsg(msg,((MultiHop)msg).offset_data(0));
			ITSMsg smsg;
			
			smsg = new ITSMsg(msg.dataGet(),0);//((MultiHop)msg).offset_data(0));
			operMode = OPER_EVERY_MSG;
			moteID = smsg.get_sid();
			moteData = smsg.get_status();
			/*
			if (smsg.get_operationMode()==OPER_EVERY_MSG)
			{
				operMode = OPER_EVERY_MSG;
				moteID = smsg.get_sid();
				moteData = smsg.get_speed();
			}
			else if (smsg.get_operationMode()==OPER_SUMMARY_MSG)
			{
				operMode = OPER_SUMMARY_MSG;
				moteID = smsg.get_sid();
				moteData = (int)smsg.get_speed();
			}
			/*
			{
				System.out.println("ITS2 received");
				ITSMsg2 smsg = new ITSMsg2(msg.dataGet(),((MultiHop)msg).offset_data(0));
